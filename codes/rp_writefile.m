function [] = rp_writefile(data,filename,isize,vsize,Header,dtype)
%Write ANALYZE 7.5 or NIFTI file by YAN Chao-Gan
% FORMAT function [] = rp_writefile(data,filename,isize,vsize,dtype)
%              data  - data file.   
...
%   Revised by YAN Chao-Gan, 090420. rp_WriteAnalyzeImage.m only recognize 'float', thus change the parameter 'single' to 'float'.
...
%     end   
    ...
    Header.dim   =isize;
    Header.pinfo = [1;0;0];
    rp_WriteNiftiImage(data,Header,filename);
else
    if strcmp(dtype,'single')
        dtype='float';  %Revised by YAN Chao-Gan, 090420. rp_WriteAnalyzeImage.m only recognize 'float', thus change the parameter 'single' to 'float'.
    end
    if strcmpi(num2str(size(vsize)),'3  1')
        vsize=vsize';   %Revised by YAN Chao-Gan, 100225. Check if vsize is a 1 by 3 array.
    end
    rp_WriteAnalyzeImage(data,filename,isize,vsize,Header.Origin,dtype);
end
